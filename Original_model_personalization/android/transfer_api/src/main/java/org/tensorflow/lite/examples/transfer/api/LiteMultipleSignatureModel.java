...
public class LiteMultipleSignatureModel implements Closeable {
   ...
   */
  float[] loadBottleneck(float[][][] image) { //bottleneck 특성 가져오기
    Map<String, Object> inputs = new HashMap<>(); //input HashMap
    inputs.put("feature", new float[][][][] {image}); //이미지 추가 [][224][224][3]
    Map<String, Object> outputs = new HashMap<>();  //output HashMap
    float[][] bottleneck = new float[1][BOTTLENECK_SIZE]; //bottleneck [1][62720]
    outputs.put("bottleneck", bottleneck);      //output 에 추가
    this.interpreter.runSignature(inputs, outputs, "load"); //model 의 load 를 통해 이미지의 특성 반환
                                                                        //output 의 bottleneck 에 저장
    return bottleneck[0]; //bottleneck 반
  }
  ...
  int getNumBottleneckFeatures() {  //train 할 때 InputTensor 를 get 함
