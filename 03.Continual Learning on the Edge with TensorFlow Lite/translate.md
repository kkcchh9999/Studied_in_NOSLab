...
추가적으로, 우리는 단순 재현 접근 방식을 현재 전이 학습 모델에 통합하는 방식으로 TensorFlow Lite 라이브러리를 확장하여 continual learning(연속학습)을 구현한다.
...
- TF Lite에 간단한 재현 버퍼를 도입하는 방식으로 지속적인 학습을 API에 통합하여 TF Lite 기능을 확장한다.  
...
이 논문에서는 그림 1에서 나타내는 간단한 재현 버퍼를 통합하여 head 신경망을 확장한다.
...
CORe 어플리케이션은 AR1 LR 알고리즘의 안드로이드 구현이고, pool6 레이어에 재현 버퍼가 내재된다. 
이는 Caffe를 사용해서 안드로이드와 C++을 위해 크로스컴파일 되었다. 
이는 500가지 패턴을 재현 버퍼에 저장하고, 사용자가 5개의 새로운 클래스를 추가할 수 있다. 
샘플이 캡처되거나 재현되면 전체 신경망이 학습될 수 있고, 따라서 지속적으로 조정하고 정확도를 높일 수 있지만, 계산 오버헤드의 단점이 존재한다.
...
우리는 TL과 동일한 모델을 사용했지만, 헤드에 재현 버퍼가 추가되었다.
전체 영상 샘플을 버퍼에 저장하는 대신 기본 모델에서 추출한 특징 패턴을 저장한다.
해당 방식을 통해 구현된 모델은 샘플을 재현하고 저장할 때 훨씬 적은 자원을 사용한다. 
우리는 head의 fully-connected 레이어에 128개의 뉴런, SGD 옵티마이저 그리고 ReLU 활성화 함수를 사용했다.
CORe50 벤치마크에서 각 배치의 훈련이 끝난 후에, 버퍼의 각 샘플이 head 모델에서 다시 재현된다.
재현 버퍼가 가득 차면, 우리는 1.5%의 현재 샘플을 새로운 것과 교체한다.    

최초에, 우리는 FIFO를 사용하여 버퍼의 특성을 교체했지만, 대략 100개의 훈련 배치 이후로 연속학습을 실패했다. 
이는 재현 버퍼에 각 클래스별 적절한 representation이 없어서 일어난 일 같다. 
FIFO 방식에서 랜덤하게 버퍼의 특성을 교체하는 방식으로 이 이슈를 해결했고, 이는 그림 3에 나타난다.
재현 버퍼에서 더 나은 클래스 표현과 더 정교한 샘플 교체 방법을 보장하면 CL 모델의 성능이 더욱 향상될 가능성이 높지만, 현재 이 논문의 범위를 벗어나 향후 작업을 위해 남겨져 있다.   

우리는 재현 버퍼의 크기와 정확도 간의 관계를 증명하기 위한 연속된 실험을 진행했고, 이는 그림 4에 나타난다.
7500 특성 패턴의 버퍼를 가지는 것이 이 벤치마크 시나리오에서 가장 좋은 균형을 이루었고, 이는 30000 버퍼에 비해 75% 적은 스토리지를 가지는 반면 정확도는 3.5%밖에 차이나지 않기 때문이다. 
그림 2에 나타난대로, 재현 버퍼를 TL 모델의 head에 통합하는 것은 배치된 TF Lite 모델에 연속학습 기능을 제공하는 좋은 첫 번째 단계이다.  

우리는 AR1 LR을 우리 모델의 upper bound 로 사용한다. 
MobileNet의 가장 낮은 레이어(pool6)에서 잠재 재현 버퍼를 낮추면 이 모델이 우리의 모델과 비교할 수 있다.
